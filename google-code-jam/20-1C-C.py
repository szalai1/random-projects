...
def case_two(slices: Counter):
    if slices.most_common(1)[0][1] >= 2:
        return 0
    return 1

def case_three(slices: Counter, maxS):
    mc = slices.most_common(1)[0]
    if mc[1]  >= 3:
        return 0
    if mc[1] == 2 and mc[0] != maxS:
        return 1
    for k in slices:
        if k%2 == 0 and slices[int(k/2)] != 0:
            return 1
    return 2
...
def main():
    ...
    for case in range(T):
        N,D = read_2_ints()
        slices = list(map(lambda x: int(x), read_str().split(" ")))
        dist = Counter(slices)
        if D == 2:
            print_ans(case, case_two(dist))
        elif == 3 :
            print_ans(case, case_three(dist, max(slices)))
        else:
            bi = dist.most_common(1)
            db = bi[1]
            size = bi[0]
            cuts = 0
            for i in range(int(math.ceil(math.log(D/bi[0]), 2))):
                if dist[size/2]:
                    extra = dist[size/2]
                    db*=2
                    db += extra
